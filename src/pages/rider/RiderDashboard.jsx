// src/pages/rider/RiderDashboard.jsx  —  Cubiny v2
// Calls fareService for live surge calc, mockService for ride requests
import { useState, useEffect } from "react";
import { MapPin, Navigation, Phone, MessageSquare, CheckCircle, Star, Car, Zap, Bike, X } from "lucide-react";
...
import { getAllFares, calcFare }  from "../../services/fareService";
    ...
    <Modal open={open} onClose={onClose} title="Rate your driver" subtitle="How was your experience with Hassan?">
      {done ? (
        ...
        <>
          ...
          {score > 0 && (
            <div style={{ textAlign:"center", marginBottom:16, fontSize:14, color:"var(--t2)" }}>
...
export function RiderDashboard() {
  ...
  // Load fares via fareService on mount
  ...
  const handleRequest = async () => {
    ...
    setSubmitting(true);
    await requestRide({ pickup, dropoff, vehicleType:selType });
    setSubmitting(false);
    setState("searching");
    setTimeout(()=>setState("found"),       2200);
    setTimeout(()=>setState("en_route"),    3800);
    setTimeout(()=>setState("in_progress"), 6500);
    setTimeout(()=>{ setState("completed"); setShowRate(true); }, 11000);
  };
      ...
      <div style={{ flex:1, position:"relative" }}>
        ...
        {!isIdle && (
          <div style={{
            ...
            <div style={{ width:8, height:8, borderRadius:"50%", background:STATES[state]?.color, boxShadow:`0 0 10px ${STATES[state]?.color}`, animation:"ping-sm 1.5s ease-out infinite" }}/>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"20px 24px", display:"flex", flexDirection:"column", gap:18 }}>
            ...
            <>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:3 }}>
                <div style={{ position:"relative" }}>
                  <div style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", width:8, height:8, borderRadius:"50%", background:"var(--c2)", zIndex:1 }}/>
                  <input placeholder="Pickup location" value={pickup} onChange={e=>setPickup(e.target.value)}
                    style={{ width:"100%", background:"var(--s2)", border:"1px solid var(--b2)", borderRadius:"var(--r2) var(--r2) 0 0", padding:"13px 16px 13px 32px", fontSize:14, color:"var(--t1)", fontFamily:"var(--font-b)" }}
                ...
                <div style={{ height:1, background:"var(--b1)", marginLeft:14 }}/>
                <div style={{ position:"relative" }}>
                  <div style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", width:8, height:8, borderRadius:2, background:"var(--v2)", zIndex:1 }}/>
                  <input placeholder="Drop-off destination" value={dropoff} onChange={e=>setDropoff(e.target.value)}
                    style={{ width:"100%", background:"var(--s2)", border:"1px solid var(--b2)", borderRadius:"0 0 var(--r2) var(--r2)", padding:"13px 16px 13px 32px", fontSize:14, color:"var(--t1)", fontFamily:"var(--font-b)" }}
                    onFocus={e=>{e.target.style.background="var(--s3)";e.target.style.borderColor="rgba(109,40,217,0.4)"}}
                    onBlur={e=>{e.target.style.background="var(--s2)";e.target.style.borderColor="var(--b2)"}}
                  />
                </div>
                  ...
                  {TYPES.map(t=>{
                    ...
                    return (
                      <button key={t.id} onClick={()=>setSelType(t.id)} style={{
                        ...
                        background: sel ? "rgba(109,40,217,0.1)" : "var(--s1)",
                        cursor:"pointer", transition:"all 0.15s",
                        boxShadow: sel ? "0 0 0 1px rgba(109,40,217,0.15), var(--sh-v)" : "none",
                      }}>
                        ...
                        <div style={{ textAlign:"right" }}>
                          <div style={{ fontSize:15, fontWeight:700, color:sel?"var(--v3)":"var(--t2)", fontFamily:"var(--font-d)" }}>
                            Rs. {f?.finalFare}
                          </div>
                          {f?.surgeApplied && (
                            <div style={{ fontSize:10, color:"var(--amb)", marginTop:2 }}>⚡ ×{f.multiplier}</div>
                          )}
                        </div>
                      </button>
                    );
              ...
              {activeFare && (
                <div style={{ background:"linear-gradient(135deg,rgba(109,40,217,0.08),rgba(8,145,178,0.05))", border:"1px solid rgba(109,40,217,0.2)", borderRadius:"var(--r2)", padding:"16px 18px" }}>
                  ...
                  <div style={{ display:"flex", gap:0, borderTop:"1px solid var(--b1)", paddingTop:10 }}>
                    {[["Base",`Rs. ${activeFare.baseFare}`],["Surge",activeFare.surgeApplied?`×${activeFare.multiplier}`:"None"],["Payment","Wallet"]].map(([k,v])=>(
                      <div key={k} style={{ flex:1, textAlign:"center" }}>
                        <div style={{ fontSize:10, color:"var(--t4)", marginBottom:2 }}>{k}</div>
                        <div style={{ fontSize:12, fontWeight:600, color:"var(--t2)", fontFamily:"var(--font-m)" }}>{v}</div>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="animate-fade-up" style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <div style={{ background:"rgba(109,40,217,0.08)", border:"1px solid rgba(109,40,217,0.2)", borderRadius:"var(--r3)", padding:20 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:14, alignItems:"center" }}>
                  <StatusPill status={state==="in_progress"?"In Progress":state==="en_route"?"Driver En Route":state==="found"?"Accepted":"Requested"}/>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:11, color:"var(--t4)", marginTop:1 }}>LEJ-3421 · White · Verified ✓</div>
                  </div>
                </div>
              ...
              {[{label:"From",val:pickup||"F-10 Markaz",dot:"var(--c2)"},{label:"To",val:dropoff||"Centaurus Mall",dot:"var(--v2)"}].map(r=>(
                <div key={r.label} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 16px", background:"var(--s1)", borderRadius:"var(--r2)", border:"1px solid var(--b1)" }}>
          ...
          {isCompleted && (
            <div style={{ textAlign:"center", padding:"24px 0" }} className="animate-bounce-in">
              ...
              <Button fullWidth onClick={()=>{setState("idle");setPickup("");setDropoff("");}}>
                Book Another Ride
              </Button>
            </div>
          )}
