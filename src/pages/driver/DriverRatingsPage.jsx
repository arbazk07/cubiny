...
export function DriverRatingsPage() {
  ...
  const isFlagged = parseFloat(avg) < RATING_THRESHOLD;
  ...
  return (
    <div className="mesh-bg" style={{ padding: 28, overflowY: "auto", height: "100vh" }}>
      ...
      {/* Trigger warning */}
      {isFlagged && (
        <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: "var(--r)", padding: "14px 18px", marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
          <span>⚠️</span>
          <div>
            <div style={{ fontWeight: 600, color: "#f87171", fontSize: 14 }}>Rating Below Threshold</div>
            <div style={{ fontSize: 12, color: "var(--clr-txt-2)" }}>
              Your average is below {RATING_THRESHOLD} stars. Your account has been flagged for admin review.
            </div>
          </div>
        </div>
      )}
      ...
      <div className="glass" style={{ padding: 24 }}>
        <h3 style={{ fontFamily: "var(--font-display)", fontSize: 15, marginBottom: 16 }}>Rider Reviews</h3>
