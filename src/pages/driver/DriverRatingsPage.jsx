// src/pages/driver/DriverRatingsPage.jsx — Cubiny v6
...
export function DriverRatingsPage() {
  ...
  return (
    <div className="page-scroll">
      ...
      <div style={{ background:'white', border:'1px solid var(--border)', borderRadius:'var(--r-xl)', padding:'24px', marginBottom:16, boxShadow:'var(--shadow-sm)', display:'flex', alignItems:'center', gap:20 }}>
        <div style={{ textAlign:'center', minWidth:80 }}>
          <p style={{ fontSize:44, fontWeight:900, color:'var(--text-primary)', letterSpacing:'-0.04em' }}>{avg}</p>
          <div style={{ display:'flex', gap:2, justifyContent:'center', margin:'4px 0' }}>
            {[1,2,3,4,5].map(i=><Star key={i} size={13} color="#F59E0B" fill={i<=Math.round(parseFloat(avg))?'#F59E0B':'transparent'} strokeWidth={1.8}/>)}
          </div>
          <p style={{ fontSize:11, color:'var(--text-muted)' }}>{ratings.length} reviews</p>
        </div>
        <div style={{ flex:1 }}>
          {[5,4,3,2,1].map(s => {
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:r.comment?10:0 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize:11, color:'var(--text-muted)' }}>{r.date}</p>
                </div>
              </div>
              <div style={{ display:'flex', gap:2 }}>
                {[1,2,3,4,5].map(s=><Star key={s} size={13} color="#F59E0B" fill={s<=r.score?'#F59E0B':'transparent'} strokeWidth={1.8}/>)}
              </div>
            </div>
